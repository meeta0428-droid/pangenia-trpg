document.addEventListener('DOMContentLoaded', () => {
    ...
    const bodyEquipInput = document.getElementById('bodyEquipInput');
    const otherModInput = document.getElementById('otherModInput');
    const totalHpDisplay = document.getElementById('totalHpDisplay');

    function calculateHP() {
        const strength = parseInt(strengthInput.value) || 0;
        const bodyEquip = parseInt(bodyEquipInput.value) || 0;
        const otherMod = parseInt(otherModInput.value) || 0;
        const total = 10 + strength + bodyEquip + otherMod;

        totalHpDisplay.textContent = total;
    }

    // Attach listeners
    [strengthInput, bodyEquipInput, otherModInput].forEach(input => {
